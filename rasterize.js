...
function loadTexture(url) {
    ...
    image.addEventListener('load', function() {
        ...
        gl.bindTexture(gl.TEXTURE_2D, data.texture);
        gl.texImage_2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, image);
    })
    ...
    try {
        if (inputTriangles == String.null)
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                textures[whichSet] = loadTexture(inputTriangles[whichSet].texture);
...
function setupShaders() {
    ...
    var vShaderCode = `
        ...
        varying vec2 vVertexUV; // interpolated texture uv for frag shader
    ...
    var fShaderCode = `
        ...
        void main(void) {
            ...
            vec4 texColor = texture2D(uvSampler, vVertexUV);
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                textureULoc = gl.getUniformLocation(shaderProgram, "uvSampler"); // ptr to texture sampler
